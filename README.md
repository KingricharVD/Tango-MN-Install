...
* First, we will do the initial collateral TX and send exactly 10000 TNGO to one of our addresses. To keep things sorted in case we setup more masternodes we will label the addresses we use.

  - Open your TNGO wallet and switch to the "Receive" tab.
  ...
  - Once all addresses are created send 10000 TNGO each to them. Ensure that you send exactly 10000 TNGO and do it in a single transaction. You can double check where the coins are coming from by checking it via coin control usually, that's not an issue.
...
Close your wallet and open the TNGO Appdata folder. Its location depends on your OS.

* **Windows:** Press Windows+R and write %appdata% - there, open the folder Tango.  
* **macOS:** Press Command+Space to open Spotlight, write ~/Library/Application Support/Tango and press Enter.  
* **Linux:** Open ~/.Tango/

In your appdata folder, open masternode.conf with a text editor and add a new line in this format to the bottom of the file:

```bash
masternodename ipaddress:8484 genkey collateralTxID outputID
```

An example would be

```
mn1 127.0.0.2:8484 93HaYBVUCYjEMeeH1Y4sBGLALQZE1Yc1K64xiqgX37tGBDQL8Xg 2bcd3c84c84f87eaa86e4e56834c92927a07f9e18718810b92e0d0324456a67c 0
```
...
```bash
bash <( curl https://raw.githubusercontent.com/tangocrypto/Tango-MN-Install/master/install.sh )
```
