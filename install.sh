...
TANGO Masternode installer arguments:
...
TARBALLURL="https://github.com/tangocrypto/Tango/releases/download/1.2.0/tango_linux.1_2_0.tar.gz"
TARBALLNAME="tango_linux.1_2_0.tar.gz"
BOOTSTRAPURL=""
BOOTSTRAPARCHIVE=""
BWKVERSION="1.2.0"
...
echo "
 ...
 | the TNGO Team CANNOT assist you in repairing  |::
...
rm $TARBALLNAME
mv ./tangod /usr/local/bin
mv ./tango-cli /usr/local/bin
mv ./tango-tx /usr/local/bin
rm -rf $TARBALLNAME

# Create .tango directory
mkdir $USERHOME/.tango
...
# Create tango.conf
touch $USERHOME/.tango/tango.conf
cat > $USERHOME/.tango/tango.conf << EOL
${INSTALLERUSED}
rpcuser=${RPCUSER}
...
masternode=1
addnode=192.250.236.139
addnode=155.138.224.172
addnode=45.77.226.105
EOL
chmod 0600 $USERHOME/.tango/tango.conf
chown -R $USER:$USER $USERHOME/.tango

sleep 1

cat > /etc/systemd/system/tango.service << EOL
[Unit]
Description=tango
...
ExecStart=/usr/local/bin/tangod -conf=${USERHOME}/.tango/tango.conf -datadir=${USERHOME}/.tango
ExecStop=/usr/local/bin/tango-cli -conf=${USERHOME}/.tango/tango.conf -datadir=${USERHOME}/.tango stop
Restart=on-abort
[Install]
